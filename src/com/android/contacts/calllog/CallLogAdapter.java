...
import android.widget.TextView;//xiaohong add 

import java.util.LinkedList;
import android.provider.Settings;//xiaohong add
import android.util.Log;//xiaohong add 
...
/*package*/ class CallLogAdapter extends GroupingListAdapter
    ...
    private void bindView(View view, Cursor c, int count) {
        ...
        final int subscription = c.getInt(CallLogQuery.SUBSCRIPTION);//xiaohong add 2013/2/23

        final ContactInfo cachedContactInfo = getContactInfoFromCallLog(c);

        views.primaryActionView.setTag(
                IntentProvider.getCallDetailIntentProvider(
                        this, c.getPosition(), c.getLong(CallLogQuery.ID), count,subscription));//xiaohong modify
        // Store away the voicemail information so we can play it directly.
        if (callType == Calls.VOICEMAIL_TYPE) {
            String voicemailUri = c.getString(CallLogQuery.VOICEMAIL_URI);
            final long rowId = c.getLong(CallLogQuery.ID);
            views.secondaryActionView.setTag(
                    IntentProvider.getPlayVoicemailIntentProvider(rowId, voicemailUri,subscription));//xiaohong modify
        } else if (!TextUtils.isEmpty(number)) {
            // Store away the number so we can call it directly if you click on the call icon.
            views.secondaryActionView.setTag(
                    IntentProvider.getReturnCallIntentProvider(number,subscription));//xiaohong modify
        } else {
            // No action enabled.
        ...
        final int durationType = c.getInt(CallLogQuery.DURATION_TYPE);//xiaohong add 
        final PhoneCallDetails details;
        if (TextUtils.isEmpty(name)) {
            details = new PhoneCallDetails(number, formattedNumber, countryIso, geocode,
                    callTypes, date, duration, subscription, durationType);//xiaohong modify
        } else {
            // We do not pass a photo id since we do not need the high-res picture.
            details = new PhoneCallDetails(number, formattedNumber, countryIso, geocode,
                    callTypes, date, duration, subscription, durationType, name, ntype, label, lookupUri, null);//xiaohong modify
        }

        final boolean isNew = c.getInt(CallLogQuery.IS_READ) == 0;
        // New items also use the highlighted version of the text.
        final boolean isHighlighted = isNew;
        mCallLogViewsHelper.setPhoneCallDetails(views, details, isHighlighted, view.getContext());//xiaohong modify 
