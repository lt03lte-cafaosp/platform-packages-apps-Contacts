    ...
    private List<AccountListItem> loadAccountFilters(Context context) {
        ...
        for (AccountWithDataSet account : accounts) {
            ...
            if (!TextUtils.isEmpty(account.type))
            {
                ...
                {
                    int sub = getSub(account.name, account.type);
                    total = getAdnCount(getSub(account.name, account.type));
                    if (total > 0) {
    ...
    public static int getSub(String name, String type) {
        ...
        if (name != null && type != null) {
            if (MSimTelephonyManager.getDefault().isMultiSimEnabled()) {
                if (SimContactsConstants.PHONE_NAME.equals(name)
                        && SimContactsConstants.ACCOUNT_TYPE_PHONE.equals(type)) {
                    sub = -1;
                }
                else if (SimContactsConstants.SIM_NAME_1.equals(name)
                        && SimContactsConstants.ACCOUNT_TYPE_SIM.equals(type)) {
                    sub = 0;
                }
                else if (SimContactsConstants.SIM_NAME_2.equals(name)
                        && SimContactsConstants.ACCOUNT_TYPE_SIM.equals(type)) {
                    sub = 1;
                }
            }
            else {
                if (SimContactsConstants.SIM_NAME.equals(name)
                        && SimContactsConstants.ACCOUNT_TYPE_SIM.equals(type)) {
                    sub = 0;
                }
            }
